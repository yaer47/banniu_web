...
YX.fn.showTeamInfo = function() {
    if (this.$teamInfoContainer.is('.hide')) {
        var that = this
		// 获取群成员
		...
		var showUI = function(){
			...
			if(that.$memberTitle)
				that.$memberTitle.html('群成员/'+members.length.toString())
			// if (type === 'normal' || myTeamInfo.type!=="normal") { // 是群主
		...
		that.$sendBtn.removeClass('bottom-right-radius')
